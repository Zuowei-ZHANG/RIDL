...
class DenseNet(nn.Module):
    def __init__(self, block, nblocks, growth_rate=12, reduction=0.5, num_classes=10):
        ...
        self.linear = nn.Linear(num_planes*1*1, num_classes) 
    ...
    def forward(self, x):
        if torch.cuda.is_available():
            out = self.conv1(x.type(torch.cuda.FloatTensor))
        else:
            out = self.conv1(x)
        ...
        #out = F.avg_pool2d(F.relu(self.bn(out)), 1)  #32*32
        out = F.avg_pool2d(F.relu(self.bn(out)), poolsize)  #128*128
        #out = F.avg_pool2d(F.relu(self.bn(out)), 7)  #224*224
